...
public class CancelAppointment extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(user==null){
			ServletUtils.showForm(request, response, "Error123123", "info.jsp");
			return;
		}else if(user instanceof Patient){
			suc = Scheduler.cancelAppointment(username,(Patient) user, appointmentDate);
			ServletUtils.showForm(request, response, suc?"Success! You cancel your appointment on "+appointmentDate : "234234Error! please try again", "info.jsp");
			return;
		...
		ServletUtils.showForm(request, response, "2342442516please try again", "info.jsp");
	}
