...
public class ScheduleAppointment extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(ServletUtils.isEmpty(docUsername) || ServletUtils.isEmpty(appointmentDate)) {
			ServletUtils.showForm(request, response, "Empty doctor or date", "showDoctor.jsp");
			return;
		}else if(!Scheduler.isDoctorAvailable(docUsername, appointmentDate)){
			ServletUtils.showForm(request, response, "Doctor "+ docUsername + " isn't available on "+ appointmentDate, "showDoctor.jsp");
			return;
		}
		if(Scheduler.scheduleAppointment(pat, docUsername, appointmentDate)){
			ServletUtils.showForm(request, response, "Success! Your new appointment scheduled on " + appointmentDate, "showDoctor.jsp");
		}else{
			ServletUtils.showForm(request, response, "2366456546fgdkf please try again", "showDoctor.jsp");
		}
	}
