...
public class Scheduler implements ServletContextListener{
	private static PreparedStatement stm1, stm2, stm3, stm4, stm5, stm6, stm7, stm8, stm9, stm10 ,stm16;
	...
	public void contextInitialized(ServletContextEvent sce) {
		try {
			...
			stm1 = con.prepareStatement(
					  "SELECT d.doctorname as name,a.d as date "
			...
			stm2= con.prepareStatement(
					  "SELECT * "
					+ "FROM ( "
					+ "	SELECT d.username,d.doctorname,  "
			...
			stm3 = con.prepareStatement(
					  "SELECT d.username as username, d.doctorname as name,a.d as date "
					+ "FROM appointments as a INNER JOIN patient as p ON p.username = a.pat_username "
					+ "INNER JOIN doctor as d ON a.doc_username = d.username "
					+ "WHERE p.username = ? AND a.d >= CURRENT_TIMESTAMP "
					+ "ORDER BY a.d;");
			stm4 = con.prepareStatement(
					  "SELECT p.username as username, p.patientname as name,a.d as date "
					+ "FROM appointments as a INNER JOIN patient as p on p.username = a.pat_username "
					+ "INNER JOIN doctor as d on a.doc_username = d.username "
			...
			stm9 = con.prepareStatement("SELECT d_start as start, d_end as end FROM availabillity WHERE username = ? AND d_start >= CURRENT_TIMESTAMP ORDER BY start; ");

			stm10 = con.prepareStatement(
					  "DELETE FROM appointments "
					+ "WHERE doc_username = ? AND pat_username = ? AND d = ?::timestamp ;");
			
                        stm16 = con.prepareStatement("SELECT doc_username,pat_username,d FROM appointments WHERE d >= CURRENT_TIMESTAMP");
	...
	public static List<Availability> getAvailableDoctors(String spec1, String start, String end) {
            List<Availability> avs = new ArrayList<>();
            try {
			
			stm2.setString(1, spec1);
			...
			while(rs.next()) {
				...
				doc.setName(rs.getString("doctorname"));
	...
	public static boolean cancelAppointment(String doctorUsername, String patientUsername, String date) {
        ...
        try {
            ArrayList<Appointment> lpa = new ArrayList<>();
            ResultSet rs = stm16.executeQuery();

            while (rs.next()) {
                Doctor dt = new Doctor();
                dt = Accounts.getInfoDoctor(rs.getString(1));
                Patient pa = new Patient();
                pa = Accounts.getInfoPatient(rs.getString(2));
                Appointment ap = new Appointment(dt,pa);
                ap.setDate(rs.getTimestamp(3));
	...
	public static boolean cancelAppointment(Doctor doc, String patientUsername, String date) {
        ...
        List<Availability> doctors =  Scheduler.getAvailableDoctors("pathologist","2022-07-18 10:00:00" , "2022-07-18 14:00:00");
         }
}
